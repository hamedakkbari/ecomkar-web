...
import { validateContactInput } from "@/lib/server/validators";
...
import type { ApiResponse, ContactRequest, WebhookPayload, SelfTestResponse } from "@/lib/types/api";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const webhookUrl = env.N8N_WEBHOOK_CONTACT;
    ...
    if (isWebhookEnabled(webhookUrl)) {
      ...
      if (webhookResult.success) {
        ...
        responseMessage = "پیام دریافت شد.";
      } else {
        const duration = Date.now() - startTime;
    ...
    // Log success
    const duration = Date.now() - startTime;
    logger.info(route, ipHash, userAgent, duration, 200, "Contact submitted successfully", {
      sanitizedData: sanitizeForLogging(body),
      responseId
    });
